...
EXCEPTION
    WHEN foreign_key_violation THEN
        RAISE EXCEPTION 'No se puede eliminar el cargo porque tiene trabajadores u otros registros asociados';
    WHEN OTHERS THEN
        RAISE EXCEPTION 'Error al eliminar cargo: %', SQLERRM;
END;
$$ LANGUAGE plpgsql;

-- Función para Listar Cargos
CREATE OR REPLACE FUNCTION fn_tab_cargos_select(p_id_cargo INTEGER DEFAULT NULL)
RETURNS TABLE (
    id_cargo INTEGER,
    nom_cargo VARCHAR(100),
    descripcion_cargo VARCHAR(255),
    nivel_riesgo_cargo VARCHAR(50),
...
BEGIN
    IF p_id_cargo IS NULL THEN
        RETURN QUERY 
        SELECT c.id_cargo, c.nom_cargo, c.descripcion_cargo, c.nivel_riesgo_cargo, c.salario_base, c.departamento 
        FROM tab_cargos c;
    ELSE
        RETURN QUERY 
        SELECT c.id_cargo, c.nom_cargo, c.descripcion_cargo, c.nivel_riesgo_cargo, c.salario_base, c.departamento 
