...
EXCEPTION
    WHEN foreign_key_violation THEN
        RAISE EXCEPTION 'No se puede eliminar la categoría porque tiene EPPs u otros registros asociados';
    WHEN OTHERS THEN
...
CREATE OR REPLACE FUNCTION fn_tab_categorias_select(p_id_categoria INTEGER DEFAULT NULL)
RETURNS TABLE (
...
BEGIN
    IF p_id_categoria IS NULL THEN
        RETURN QUERY 
        SELECT c.id_categoria, c.nom_categoria, c.descripcion_categoria 
