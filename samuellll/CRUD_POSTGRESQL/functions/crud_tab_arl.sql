...
-- Función para Listar ARL
CREATE OR REPLACE FUNCTION fn_tab_arl_select(p_id_arl INTEGER DEFAULT NULL)
RETURNS TABLE (
    id_arl INTEGER,
    nom_arl VARCHAR(100),
    nit_arl VARCHAR(20),
    direccion_arl VARCHAR(200),
    tel_arl VARCHAR(15),
    correo_arl VARCHAR(100)
) AS $$
BEGIN
    IF p_id_arl IS NULL THEN
        RETURN QUERY 
        SELECT a.id_arl, a.nom_arl, a.nit_arl, a.direccion_arl, a.tel_arl, a.correo_arl 
        FROM tab_arl a;
    ELSE
        RETURN QUERY 
        SELECT a.id_arl, a.nom_arl, a.nit_arl, a.direccion_arl, a.tel_arl, a.correo_arl 
        FROM tab_arl a 
        WHERE a.id_arl = p_id_arl;
    END IF;
END;
$$ LANGUAGE plpgsql;
